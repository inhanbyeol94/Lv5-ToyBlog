...
class MemberService {
  ...
  createMember = async (req) => {
    try {
      ...
      if (overlapId) return { code: 405, message: '이미 사용중인 아이디입니다.' };

      const overlapNickname = await this.memberRepository.findUser({ nickname: nickname });
      if (overlapNickname) return { code: 405, message: '이미 사용중인 닉네임입니다.' };
  ...
  login = async (req, res) => {
    try {
      ...
      if (!findUser) return { code: 401, message: '아이디와 패스워드가 일치하지 않습니다.' };
